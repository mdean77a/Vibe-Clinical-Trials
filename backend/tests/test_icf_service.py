...
class TestICFGenerationService:
    ...
    @pytest.mark.unit
    @pytest.mark.ai_service
    async def test_generate_icf_async_success(
        ...
        with patch.object(service, "_generate_icf_sync", return_value=expected_result):
            result = await service.generate_icf_async(
                collection_name, sample_protocol_metadata
            )
    ...
    async def test_generate_icf_async_error(self, mock_qdrant_client):
        ...
        with patch.object(
            service, "_generate_icf_sync", side_effect=Exception("Generation failed")
        ):
    ...
    def test_generate_icf_sync_missing_sections(self, mock_qdrant_client):
        ...
        with (
            ...
            with pytest.raises(
                DocumentGenerationError, match="Missing required ICF sections"
            ):
