...
class TestGetProtocolByCollectionName:
    ...
    def test_get_protocol_by_collection_name_success(self, init_test_db):
        ...
        retrieved_protocol = get_protocol_by_collection_name(created_protocol.collection_name)
    ...
    def test_get_protocol_by_collection_name_not_found(self, init_test_db):
        ...
        assert "Protocol with collection name nonexistent_collection not found" in str(exc_info.value)
...
class TestGetAllProtocols:
    ...
    @pytest.mark.unit
    def test_get_all_protocols_multiple_protocols(self, init_test_db, multiple_protocols_data):
    ...
    def test_get_all_protocols_with_status_filter(self, init_test_db):
        ...
        protocol1 = create_protocol(ProtocolCreate(
            study_acronym="STUDY-001",
            protocol_title="Processing Protocol"
        ))
        
        protocol2 = create_protocol(ProtocolCreate(
            study_acronym="STUDY-002", 
            protocol_title="Processed Protocol"
        ))
...
class TestRowToProtocol:
    ...
    def test_row_to_protocol_conversion(self, init_test_db, db_connection):
        ...
        # Retrieve the row
        cursor = db_connection.execute("SELECT * FROM protocols WHERE id = ?", (cursor.lastrowid,))
        row = cursor.fetchone()
...
class TestDatabaseConnectionManager:
    ...
    def test_get_db_connection_rollback_on_error(self, init_test_db):
        ...
        with patch('sqlite3.connect', return_value=mock_conn):
            with patch.object(mock_conn, 'execute', side_effect=sqlite3.Error("Test error")):
                try:
                    with get_db_connection() as conn:
...
class TestIntegrationScenarios:
    ...
    def test_full_protocol_lifecycle(self, init_test_db):
        ...
        retrieved_by_collection = get_protocol_by_collection_name(created_protocol.collection_name)
    ...
    def test_concurrent_protocol_creation(self, init_test_db):
        ...
        protocols_data = [
            ProtocolCreate(study_acronym=f"STUDY-{i:03d}", protocol_title=f"Protocol {i}")
            for i in range(1, 6)
        ]
