...
from ..services.icf_service import get_icf_service, ICFGenerationService
...
class ProtocolSummaryResponse(BaseModel):
    ...
    protocol_metadata: Optional[Dict[str, Any]] = Field(None, description="Protocol metadata")
...
async def generate_icf(
    ...
    **Process:**
    ...
    """
    try:
        logger.info(f"ICF generation requested for collection: {request.protocol_collection_name}")
        ...
        logger.info(f"ICF generation completed for collection: {request.protocol_collection_name}")
        ...
        raise HTTPException(status_code=500, detail="Internal server error during ICF generation")
...
async def generate_icf_stream(
    ...
    async def stream_generator():
        try:
            logger.info(f"Streaming ICF generation for collection: {request.protocol_collection_name}")
            ...
            if not collection_exists:
                error_data = {
                    "event": "error", 
                    "data": {"error": f"Protocol collection '{request.protocol_collection_name}' not found"}
                }
...
async def regenerate_icf_section(
    ...
    """
    try:
        logger.info(f"Section regeneration requested: {request.section_name} for collection: {request.protocol_collection_name}")
        ...
        valid_sections = ["summary", "background", "participants", "procedures", "alternatives", "risks", "benefits"]
        ...
        raise HTTPException(status_code=500, detail="Internal server error during section regeneration")


@router.get("/protocol/{collection_name}/summary", response_model=ProtocolSummaryResponse)
async def get_protocol_summary(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve protocol summary")
