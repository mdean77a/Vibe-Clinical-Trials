...
class PDFProcessor:
    ...
    def process_pdf_from_bytes(
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Successfully processed PDF: {len(chunks)} chunks extracted"
                )
        ...
        try:
            ...
            if hasattr(doc, "texts") and doc.texts:
                ...
                for text_element in doc.texts:
                    ...
                    if (
                        current_chunk
                        and len(current_chunk) + len(text_content) > chunk_size_limit
                    ):
            ...
            if not chunks and hasattr(doc, "pages"):
                for page in doc.pages:
                    ...
                    if hasattr(page, "texts"):
                        page_text = "\n".join(
                            [t.text for t in page.texts if t.text.strip()]
                        )

                    if page_text.strip():
                        # Split large pages into smaller chunks
                        page_chunks = self._split_text_into_chunks(
                            page_text, max_size=1000
                        )
                ...
                try:
                    ...
                    if markdown_content:
                        chunks = self._split_text_into_chunks(
                            markdown_content, max_size=1000
                        )
                    ...
                    if hasattr(result, "document") and hasattr(
                        result.document, "get_text"
                    ):
                        raw_text = result.document.get_text()
                        if raw_text:
                            chunks = self._split_text_into_chunks(
                                raw_text, max_size=1000
                            )
    ...
    def _split_text_into_chunks(
        self, text: str, max_size: int = 1000, overlap: int = 100
    ) -> List[str]:
        """
    ...
    def _extract_document_metadata(
        self, result: ConversionResult, filename: str
    ) -> Dict[str, Any]:
        """
    ...
    def _identify_document_type(self, result: ConversionResult) -> str:
        ...
        try:
            ...
            if hasattr(result.document, "texts"):
                text_content = " ".join(
                    [t.text.lower() for t in result.document.texts[:10]]
                )  # First 10 text elements

            # Look for common clinical trial document indicators
            if any(
                term in text_content
                for term in ["protocol", "clinical trial", "study protocol"]
            ):
                return "clinical_trial_protocol"
            elif any(
                term in text_content for term in ["informed consent", "consent form"]
            ):
