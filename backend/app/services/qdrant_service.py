...
class QdrantService:
    ...
    def get_collection_name_for_protocol(self, protocol_id: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Error getting collection name for protocol {protocol_id}: {e}")
            return None
    ...
    def update_protocol_status(self, collection_name: str, status: str) -> bool:
        ...
        try:
            ...
            if point_ids:
                ...
                logger.info(f"Updated status to {status} for collection {collection_name}")
                return True
            return False
    ...
    def search_protocol_documents(self, protocol_collection_name: str, query: str, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error searching protocol documents in {protocol_collection_name}: {e}")
    ...
    def get_embeddings(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            logger.info(f"Generated {len(result_embeddings)} embeddings using {self.embedding_model}")
