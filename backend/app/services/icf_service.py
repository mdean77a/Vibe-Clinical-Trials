...
class ICFGenerationService:
    ...
    async def generate_icf_async(
        ...
        """
        try:
            logger.info(f"Starting async ICF generation for collection: {protocol_collection_name}")
            ...
            logger.info(f"Completed async ICF generation for collection: {protocol_collection_name}")
            return result
    ...
    async def generate_icf_streaming(
        ...
        try:
            ...
            logger.info(f"🚀 Starting streaming ICF generation for collection: {protocol_collection_name} at {time.strftime('%H:%M:%S')}")
            ...
            if not context:
                logger.warning(f"No context found for collection: {protocol_collection_name}")
                context = [{"text": "No specific protocol context available", "score": 0.0}]
            ...
            workflow_task = asyncio.create_task(
                self._execute_streaming_workflow(streaming_workflow, workflow_inputs, event_queue)
            )
            ...
            logger.info(f"Streaming ICF generation completed for collection: {protocol_collection_name}")
    ...
    def _generate_section_streaming_sync(
        ...
        async def stream_section():
            try:
                ...
                query = section_queries.get(section_name, f"{section_name} informed consent")
                context = self.document_generator.get_protocol_context(
                    protocol_collection_name,
                    query,
                    min_score=0.3
                ...
                if not context:
                    context = [{"text": f"No specific context available for {section_name}", "score": 0.0}]
                ...
                async for chunk in self._generate_section_with_streaming_llm(
                    ...
                    context="\n\n".join([item.get("text", "") for item in context[:3]]),  # Top 3 items
    ...
    def _generate_icf_sync(
        ...
        try:
            ...
            logger.info(f"Retrieved {len(context)} context items for {protocol_collection_name}")
            for i, item in enumerate(context[:3]):
                text = item.get("text", "")
                score = item.get("score", 0)
                logger.info(f"Context {i+1}: score={score:.3f}, length={len(text)}, preview='{text[:100]}...'")
            ...
            required_sections = [
                "summary", "background", "participants", 
                "procedures", "alternatives", "risks", "benefits"
            ]
            
            missing_sections = [s for s in required_sections if s not in sections]
            if missing_sections:
                error_msg = f"Missing required ICF sections: {', '.join(missing_sections)}"
            ...
            logger.info(f"ICF generation successful: {len(sections)} sections generated")
            return response
    ...
    async def regenerate_section_async(
        ...
        """
        try:
            logger.info(f"Starting async section regeneration: {section_name} for collection: {protocol_collection_name}")
